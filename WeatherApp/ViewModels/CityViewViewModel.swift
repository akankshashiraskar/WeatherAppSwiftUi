...
final class CityViewViewModel: ObservedObject {
    ...
    var windSpeed: String {
        return String(format: "o.1f", weather.current.wind_speed)
    }
    ...
    private func getWeather(coord: CLLocationCoordinate2D?) {
        if let coord = coord {
            let urlString = API.getURLFoe(lat: coord.latitude, lon: coord.longitude)
            getWeatherInternal(city: city, for: urlString)
        } else {
            let urlString = API.getURLFoe(lat: 37.5485, lon: -121.9886)
            getWeatherInternal(city: city, for: urlString)
        }
    }
    ...
    //
    func getLotteiAnimationFor(icon: String) -> {
        switch icon {
        case "01d":
            return "dayClaearSky"
        case "01n":
            return "nightClaearSky"
        case "02d":
            return "dayFewClouds"
        case "02n":
